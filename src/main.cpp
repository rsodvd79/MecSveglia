...
static const ScreenOption SCREEN_OPTIONS[] = {
    {0, "Wi-Fi"},    {1, "Ora/Data"},        {2, "Eyes"},
    {3, "Meteo"},    {4, "Game of Life"},    {5, "Triangolo"},
    {6, "RSS ANSA"}, {7, "Ciclo automatico"}};
...
void handleRoot() {
  ...
  if (LittleFS.exists("/INDEX.HTM")) {
    ...
    if (fhtm) {
      ...
      // Sostituzione dinamica di "mdns" con il contenuto di mdns_nm.txt /
      // MDNS_NM.TXT
      // Use cached mdns_name instead of reading file every time
      if (mdns_name.length()) {
  ...
  String body;
  body.reserve(512);
  body += F("<!doctype html><html lang=\"it\"><head><meta "
            "charset=\"utf-8\"><meta name=\"viewport\" "
            "content=\"width=device-width,initial-scale=1\"><title>404 • "
            "INDEX.HTM mancante</title><style>body{font-family:system-ui,Segoe "
            "UI,Roboto,Arial;margin:0;padding:24px;background:#0f172a;color:#"
            "e5e7eb}a{color:#93c5fd}</style></head><body>");
  body += F("<h1>Pagina non trovata</h1><p>Manca il file "
            "<code>INDEX.HTM</code> su LittleFS.</p><ul>");
  body += F("<li>Carica i file in <code>/data</code> con ‘Upload Filesystem "
            "Image’.</li>");
  body += F("<li>Oppure vai a <a href=\"/setup\">/setup</a> per configurare il "
            "Wi‑Fi.</li></ul></body></html>");
...
void showScreen0(bool cmp) {
  ...
  if (cmp) {
    ...
    display.print(rssi);
    display.print(F(" dBm "));
    display.print(dBmtoPercentage(rssi));
    display.println(F(" %"));
  }
  display.display();
}
...
void showScreen1() {
  // gettimeofday(&tv, nullptr); // Removed redundant call
...
void showScreen3() {
  if (!meteo.lock(100 / portTICK_PERIOD_MS))
    return; // Try to lock, skip if busy

  display.clearDisplay();
  display.setCursor(0, 0);
  display.setTextSize(1);
  display.print(F("TEMP. "));
  display.printf("%.1f", meteo.temp);
  display.println(F(" C"));
  display.print(F("UMID. "));
  display.printf("%d", meteo.humidity);
  display.println(F(" %"));
  display.print(F("VENTO "));
  display.printf("%.1f", meteo.wind_speed);
  ...
  if (meteo.icon == F("01D")) { // clear sky
  ...
  } else if (meteo.icon == F("03D") ||
             meteo.icon == F("03N")) { // scattered clouds
    display.drawBitmap(128 - 32, 0, Clouds, 32, 32, 1);

  } else if (meteo.icon == F("04D") ||
             meteo.icon == F("04N")) { // broken clouds
    display.drawBitmap(128 - 32, 0, Clouds, 32, 32, 1);
    ...
    display.print(F(" "));
    display.printf("%.1f", meteo.snow);
    display.print(F(" mm/h"));
...
// Animated triangle: 3 independent vertices bounce with slight random angle
// change
void showScreen5() {
...
void showScreen7() {
  if (!rss.lock(100 / portTICK_PERIOD_MS))
    return; // Try to lock
  ...
  if (count == 0) {
    ...
    display.setTextSize(1);
    display.setTextWrap(
        false); // We handle wrapping manually or just let it clip
    display.setCursor(0, 0);
    display.print(F("RSS vuoto"));
    display.display();
    rss.unlock(); // Ensure unlock
    return;
  }
  ...
  const int charsPerLine = 21; // Approx for 128px width / 6px char width
  const int lineHeight = 8;
  const int gap = 4; // Space between items
  ...
  // Draw items until we fill the screen
  while (y < displayH && drawnItems < count + 1) {
    ...
    for (int line = 0; line < lines; ++line) {
      int lineY = (int)(y + line * lineHeight);
      if (lineY > -lineHeight && lineY < displayH) {
        int start = line * charsPerLine;
        int end = start + charsPerLine;
        if (end > headline.length())
          end = headline.length();

        display.setCursor(0, lineY);
        display.print(headline.substring(start, end));
      }
    }
  ...
  // Update scrolling
  // Calculate height of the top item to know when to switch
  String topHeadline = rss.item(topIndex);
  int topLines = (topHeadline.length() + charsPerLine - 1) / charsPerLine;
  if (topLines < 1)
    topLines = 1;
  int topItemHeight = topLines * lineHeight + gap;

  // Scroll speed: 1 pixel per call (approx)
  // Adjust this value to change speed.
  // showRssScrollNow period is 120ms. 1px/120ms = ~8px/sec.
  float step = 1.0f;

  scrollOffset -= step;
  if (scrollOffset <= -topItemHeight) {
    scrollOffset += topItemHeight;     // Adjust offset
    topIndex = (topIndex + 1) % count; // Move to next item
  }
...
void setup(void) {
  ...
  server.begin();
  display.println(F("HTTP server start"));
  display.display();
...
void loop(void) {
  ...
  case 3:
    if (showMeteoNow) {
      digitalWrite(LED_BUILTIN, LOW);
      // meteo.Update(); // Moved to networkTask
      digitalWrite(LED_BUILTIN, HIGH);
      showScreen3();
    }
    break;
...
void handleSetupGet() {
  ...
  body += F("<!doctype html><html lang=\"it\"><head><meta "
            "charset=\"utf-8\"><meta name=\"viewport\" "
            "content=\"width=device-width,initial-scale=1\"><title>404 • "
            "SETUP.HTM mancante</title><style>body{font-family:system-ui,Segoe "
            "UI,Roboto,Arial;margin:0;padding:24px;background:#0f172a;color:#"
            "e5e7eb}label{color:#9ca3af}input{display:block;margin-top:6px}</"
            "style></head><body>");
  body += F("<h1>Setup non disponibile</h1><p>Manca il file "
            "<code>SETUP.HTM</code> su LittleFS.</p><p>Consigli:</p><ul>");
  body += F("<li>Carica i file in <code>/data</code> con ‘Upload Filesystem "
            "Image’.</li>");
  body += F("<li>Se sei in AP, collega a SSID <strong>");
  body += mdns_name;
  body += F("</strong> e visita <a href=\"/\">Home</a>.</li></ul>");
  body += F("</body></html>");
  server.send(404, F("text/html"), body);
  digitalWrite(LED_BUILTIN, HIGH);
}
...
void handleSetupPost() {
  ...
  if (!LittleFS.begin())
    ok = false;
  if (ok && nsid.length()) {
    File f = LittleFS.open("/WIFI_SID.TXT", "w");
    if (f) {
      f.print(nsid);
      f.close();
    } else
  ...
  if (ok && npwd.length()) {
    File f = LittleFS.open("/WIFI_PWD.TXT", "w");
    if (f) {
      f.print(npwd);
      f.close();
    } else
  ...
  if (ok && nmdn.length()) {
    File f = LittleFS.open("/MDNS_NM.TXT", "w");
    if (f) {
      f.print(nmdn);
      f.close();
    } else
  ...
  if (LittleFS.exists("/favicon.ico")) {
    ...
    if (f) {
      server.sendHeader(F("Cache-Control"),
                        F("public, max-age=604800, immutable"));
